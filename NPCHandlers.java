...
public class NPCHandlers {
	...
	public static Character talkToShark(Character player){
		...
		boolean choice = player.getInput().yesNo(player);
		if (choice){
			...
			boolean confirm = player.getInput().yesNo(player);
			if (confirm){
	...
	public static Character talkToGuardDog(Character player, Items items) {
		...
		String plan = player.getInput().getInput();
		switch (plan) {
		case "fight":
		...
		case "trick":
			int trickcounter = 0;
			if (player.getInt() + player.getLvl() + player.getInput().dice(1,10) > 25 + 5 * trickcounter){
			...
			break;
		case "con":
			if (player.getCha() + player.getLvl() + player.getInput().dice(1,10) >= 25){
	...
	public static Character talkToIce(Character player) {
		...
		String reply = player.getInput().getInput();
		switch (reply) {
		case "no":
		...
		case "heal":
			if(player.getHP() < player.getMaxHP()){
				...
				++IceCounter;
					...
					boolean moveIn = player.getInput().yesNo(player);
					if (moveIn){
	...
	public static Character talkToHellhound(Character player){
		...
		case "t":
			...
			String tell = player.getInput().getInput();
			switch (tell){
			case "okay":
			case "ok":
			case "o":
	...
	public static Character talkToGuardCat(Character player, Items items) {
		...
		String choice = player.getInput().getInput();
		switch (choice){
		case "him":
		case "h":
