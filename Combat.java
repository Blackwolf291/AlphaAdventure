...
public class Combat {
	...
	public static Character combat(Character player, Items items){
		...
		while (player.getCombat()){
			...
			String action = player.getInput().getInput();
			switch (action){
			case "attack":
			case "a":
				boolean hit;
				playerTurn = false;
				int attackDie = player.getInput().dice(1,6);
				switch (attackDie){
				case 6:
					hit = true;
					break;
			...
			case "f":
				if (player.getEnemy().getCanFlee()){
					playerTurn = false;
					boolean escape;
				int escapeDie = player.getInput().dice(1,6);
				switch (escapeDie){
				case 6:
					escape = true;
			...
			case "inventory":
					...
					String item = player.getInput().getInput();
			...
			if (creatureTurn){
				Attack currentAttack = player.getEnemy().getAttack().get(player.getInput().dice(1,player.getEnemy().getAttack().size()));
		...
		if (win){
			...
			if (player.getInput().dice(1,100) >= player.getEnemy().getDropChance()){
